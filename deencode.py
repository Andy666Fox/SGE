...
def encode(file: str):  # --> PIL.PngImagePlugin.PngImageFile
    ...
    for elem in tqdm(s_arr):
        ...
        # Salt generated here
        salt_pos = "".join(
            [np.random.choice(buffer_symbols) for _ in range(6 - len(str(elem)))]
        )
        salt_neg = "".join(
            [np.random.choice(buffer_symbols) for _ in range(6 - len(str(elem)))]
        )
    ...
    p_arr = np.array(
        [list(hex2rgb(x)) for x in new_arr]
        + [[0, 0, 0] for x in range(delta_res - 1)]
        + [[srate_rgb, srate_rgb, srate_rgb]]
    )
